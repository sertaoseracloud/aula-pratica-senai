...
E o mesmo `verificar_ambiente.py` das outras aulas de PySpark, com duas
checagens a mais no fim: o boto3 instalado, e o floci respondendo na porta
4566. As quatro primeiras (Python, PySpark, Java, winutils) sao identicas
-- o Spark aqui continua rodando local[4] na sua maquina.

Nao sobe sessao Spark nem conecta no S3 de verdade alem de um teste de
socket: roda em ~1 s.
"""
...
try:
    ...
    relatar(OK, "boto3", boto3.__version__)
except ImportError:
    relatar(
        ERRO,
        "boto3",
...
# 6. floci respondendo ----------------------------------------------------------
try:
    from comum import AWS_ENDPOINT_URL

    if AWS_ENDPOINT_URL is None:
        relatar(AVISO, "floci", "AWS_ENDPOINT_URL vazia -- usando a AWS real, nao o floci")
    else:
        endpoint = urlparse(AWS_ENDPOINT_URL)
        host, porta = endpoint.hostname or "127.0.0.1", endpoint.port or 4566
        with socket.create_connection((host, porta), timeout=2):
            relatar(OK, "floci", f"respondendo em {host}:{porta}")
except ImportError:
    relatar(AVISO, "floci", "nao foi possivel importar comum.py para checar")
except OSError:
    relatar(
        ERRO,
        "floci",
...
print("Ambiente pronto. Comece por: python 00_conectar_s3.py")
